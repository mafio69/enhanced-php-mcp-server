-- MCP PHP Server User System Migration
-- Version: 001
-- Description: Create users, user_secrets, and user_activities tables
...
INSERT IGNORE INTO `users` (
    ...
    'admin@mcp-server.local',
    '$argon2id$v=19$m=65536,t=4,p=1$NGVhM2JkZjZkZjA2NzJhMA$hF9Hq2z7QJ8s3vK8n2f8VpX3kR5WqY7tU9nL4xM5sK',
...
INSERT IGNORE INTO `users` (
    ...
    'user@mcp-server.local',
    '$argon2id$v=19$m=65536,t=4,p=1$bWU3ZjRkZjZkZjA2NzJhMA$hF9Hq2z7QJ8s3vK8n2f8VpX3kR5WqY7tU9nL4xM5sK',
    'Regular User',
    'user',
    TRUE,
...
ALTER TABLE `users` ADD INDEX `idx_preferences_role` ((CAST(`preferences` AS CHAR(20))), `role`);
ALTER TABLE `user_secrets` ADD INDEX `idx_access_list_size` ((JSON_LENGTH(`access_list`)));
ALTER TABLE `user_activities` ADD INDEX `idx_details_action` ((JSON_UNQUOTE(JSON_EXTRACT(`details`, '$.action'))));
...
-- Create stored procedure for cleanup expired sessions
DELIMITER //
...
-- Create stored procedure for cleanup old activities
DELIMITER //
...
BEGIN
    ...
    SELECT ROW_COUNT() as activities_cleaned;
END //
DELIMITER ;

-- Create trigger to automatically update user last activity
DELIMITER //
...
-- Set up automatic cleanup event (MySQL 5.1.6+)
-- This event will run daily to clean up expired sessions and old activities
...
BEGIN
    ...
    CALL CleanupOldActivities(90); -- Keep activities for 90 days
END;

-- Grant necessary permissions (adjust as needed for your setup)
-- GRANT SELECT, INSERT, UPDATE, DELETE ON mcp_php_server.* TO 'mcp_user'@'localhost';
-- GRANT EXECUTE ON PROCEDURE mcp_php_server.CleanupExpiredSessions TO 'mcp_user'@'localhost';
-- GRANT EXECUTE ON PROCEDURE mcp_php_server.CleanupOldActivities TO 'mcp_user'@'localhost';

-- Migration completed successfully
SELECT 'Migration 001: Users and secrets tables created successfully' as status;